...
sumDepthHelper (Node x left right) d currentDepth
| currentDepth == d && x > 5 = x + sumDepthHelper left d (currentDepth + 1) + sumDepthHelper right d (currentDepth + 1)
= sumDepthHelper left d (currentDepth + 1) + sumDepthHelper right d (currentDepth + 1)
...
Aux1 :: [MenuItem] -> Real
Aux1 ls = toReal (sum [x.price \\ x <- ls]) / toReal (length ls)

averageRating :: [Restaurant] -> Real
averageRating [] = 0.0
averageRating rests =  toReal (sum [x.rating \\ x <- rests]) / toReal (length rests)

expensiveRestaurant :: [Restaurant] -> (String, Real)
expensiveRestaurant [] = ("", 0.0)
expensiveRestaurant rests = result
where averages = [(Aux1 r.menu, r.restname ) \\ r <- rests]
   	  maxAverage = maxList (map fst averages)
      result = hd [ (name, avgList) \\ (avg, name) <- averages ]
      avgList = toReal (sum [ x.rating \\ x <- rests]) / toReal (length rests)
