...
Occ :: Int {Int} -> Int
Occ n ls = length[x\\x<-:ls|x==n]

removeOcc :: {Int} -> {Int}
removeOcc ls = {el\\el<-:ls|isOdd(Occ el ls)}
...
repeatN :: Int Int -> [Int]
repeatN n 0 = []
repeatN n i = [n: repeatN n (i-1)]

generate :: Int -> {Int}
generate n = {y\\y<-flatten[repeatN i i\\i<-[1..n]]}
...
price :: {Product} -> Real
price ls = toReal(sum[x.price\\x<-:ls])/toReal(length[x\\x<-:ls])

cheapestShop :: {Shop} -> (String,City)
cheapestShop ls = hd[(x.shopName,x.location)\\x<-:ls|price x.products==minList list]
where list = [price x.products\\x<-:ls]
...
remove vector a
| isMember a vector = [x\\x<-vector|x<>a]
...
indexOf vector a
| isMember a vector = hd[i\\i<-[0..length vector-1]|vector!!i==a]
= abort "Element does not exist"
...
swapAux :: (Vector a) a a -> (Vector a) | Eq a
swapAux [] _ _ = []
swapAux [x:xs] a b
| x == a = [b: swapAux xs a b]
| x == b = [a: swapAux xs a b]
= [x: swapAux xs a b]

swap :: (Vector a) a a -> (Vector a) | Eq a
swap vector a b
| isMember a vector && isMember b vector = swapAux vector a b 
= abort "Element does not exist"
...
messR a [] = a
messR [] b = b
messR [x:xs] [y:ys] = [x,y] ++ messR xs ys 
...
instance Merge Int
where
	 mess ls1 ls2 = messR ls1 (reverse ls2)
	 sorted a b = sort (mess a b)
	 Empty = []
...
:: BST a = Leaf | Node a (BST a) (BST a)

insert :: a (BST a) -> (BST a) | Ord a
insert n Leaf = Node n Leaf Leaf
insert n (Node x l r)
| x >= n    = Node x (insert n l) r
= Node x l (insert n r)

bsearch :: [a] -> (BST a) | Ord a 
bsearch xs = foldl (flip insert) Leaf xs

bcollect :: (BST a) -> [a] | Ord a
bcollect Leaf = []
bcollect (Node x l r) = bcollect l ++ [x] ++ bcollect r

//Start = bcollect (bsearch [29,36,38,33,18,24,10]) // [10,18,24,29,33,36,38]
...
flexTreeToList (UnaryNode x l) = [x] ++ flexTreeToList l
