...
shiftCipher str n = {toChar(((toInt char+n+65)rem 26)+65)\\char<-:str}
...
check circle ls = not(and[sqrt((p.x-circle.center.x)^2.0+(p.y-circle.center.y)^2.0)>circle.radius\\p<-:ls])
...
instance + Library
where
	 (+) a b = {lib_name=a.lib_name+++b.lib_name,books={i\\i<-sort(removeDup([x\\x<-:a.books]++[y\\y<-:b.books]))}}
...
instance == Library
where
	(==) lib1 lib2 = and[x==y\\x<-:lib1.books&y<-:lib2.books] && size(lib1.books)==size(lib2.books)
...
alternative :: [Char] [Char] -> String
alternative [] [] = ""
alternative x [] = toString x
alternative [] y = toString y
alternative [x:xs] [y:ys] = toString [x,y] +++ alternative xs ys

instance + {String}
where
	(+) str1 str2 = {alternative [i\\i<-:x] [j\\j<-:y]\\x<-:str1&y<-:str2}
...
AVL_prop_check (Node x l r) = abs (height l - height r) <= 1 && AVL_prop_check l && AVL_prop_check r
...
sumTree n list = sum(filter (\x = x > n) (tolist list))
...
totree :: [Int] -> (Tree Int)
totree [] = Leaf
totree [x:xs] = Node x Leaf (totree xs)

transform :: (Tree Int) -> (Tree Int)
transform tree = totree[x+sumTree x tree\\x<-tolist tree]
...
smallest :: {(Int,Int,Int)} -> (Int,Int,Int)
smallest ls = hd[x\\x<-:ls|minList sumls==abs(fst3 x+snd3 x+thd3 x)]
where sumls = [abs(fst3 x+snd3 x+thd3 x)\\x<-:ls|fst3 x<>snd3 x&&fst3 x<>thd3 x&&snd3 x<>thd3 x]

multiply :: (Int,Int,Int) (Int,Int,Int) -> (Int,Int,Int)
multiply x y = (fst3 x*fst3 y,snd3 x*snd3 y,thd3 x*thd3 y)

alterArray :: {(Int, Int, Int)} -> {(Int, Int, Int)}
alterArray ls = {multiply (smallest ls) x\\x<-:ls}
...
onlySurname :: String -> String
onlySurname str = {x\\x<-drop 1 (dropWhile((<>)' ') [char\\char<-:str])}

getOldestMarriage :: {(EngagedPerson, EngagedPerson)} -> String
getOldestMarriage ls = onlySurname (hd[(fst x).fullName \\ x <-: ls | (fst x).yearOfMarriage == minList list])
where list = [(fst x).yearOfMarriage \\ x <-: ls]
...
neighbours ls = [toColor((fromColor el+2)rem 7) \\ el <- ls]
...
getHeight :: BinaryTree2 -> Int
getHeight BTLeaf2 = 1
getHeight (BTNode2 l r) = 1 + max (getHeight l) (getHeight r)

getBTDiameter :: BinaryTree2 -> Int
getBTDiameter BTLeaf2 = 1
getBTDiameter (BTNode2 l r) = 1 + max (getHeight l + getHeight r) (max (getBTDiameter l) (getBTDiameter r))
