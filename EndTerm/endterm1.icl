...
avgGrades :: [Int] -> Real
avgGrades ls = toReal(sum ls)/toReal(length ls)

getBestBScStudent :: [Student] -> String
getBestBScStudent ls 
| isEmpty ls = "ERROR"
= hd[x.id\\x<-ls|maxList list==avgGrades(x.grades)]
where list = [avgGrades x.grades\\x<-ls|BSc==x.level]
...
occ :: Char String -> Int
occ ch str = length[x\\x<-:str|ch==x]

count :: String -> {(Char, Int)}
count str = {(char,occ char str)\\char<-removeDup[x\\x<-:str]|occ char str>1}
...
Insert (Pointer x xs) str = Pointer x (Insert xs str)
...
tolist Nil = []
tolist (Pointer x xs) = [x] ++ tolist xs

fromlist :: [String] -> (LinkedList String)
fromlist [] = Nil
fromlist [x:xs] = Pointer x (fromlist xs)

Reverse :: (LinkedList String) -> (LinkedList String)
Reverse list = fromlist (reverse (tolist list))
...
delete Nil str = Nil
delete (Pointer x xs) str
| x == str = delete xs str
= Pointer x (delete xs str)
...
concat ls1 ls2 = fromlist (tolist ls1 ++ tolist ls2)
...
removeAux [] ls = []
removeAux [x:xs] ls
| isMember x ls = [0] ++ removeAux xs [x:ls]
= [x] ++ removeAux xs [x:ls]

findFakes :: {BasicPersonAccount} -> {BasicPersonAccount}
findFakes ls = {x\\x<-:ls&i<-[0..size ls-1]|x.ip==hd(drop i iplist)}
where iplist = removeAux[x.ip\\x<-:ls] []
...
diagSum :: [[Int]] -> Int
diagSum ls = sum[ls!!i!!j\\i<-[0..length ls-1]&j<-[0..length ls-1]|i==j]

largestMat :: {[[Int]]} -> [[Int]]
largestMat ls = flatten[x\\x<-:ls|diagSum x == maxList list]
where list = [diagSum x\\x<-:ls]
...
tomyString :: (MyList Int) -> [Int]
tomyString Empty = []
tomyString (Elem x xs) = [x] ++ tomyString xs

string :: [Int] -> String
string [] = ""
string [x] = toString x
string [x:xs] = toString x +++ "," +++ string xs  

instance toString (MyList Int)
where
	toString Empty = "[]"
	toString list = "[" +++ (string (tomyString list)) +++ "]"
...
firstAndMiddle Leaf = []
firstAndMiddle (Node (FirstName x) a b) = [x] ++ firstAndMiddle a ++ firstAndMiddle b
firstAndMiddle (Node (MiddleName x) a b) = [x] ++ firstAndMiddle a ++ firstAndMiddle b
firstAndMiddle (Node _ a b) = firstAndMiddle a ++ firstAndMiddle b
...
findWhich [] = 0
findWhich [(A str):xs] = size str + findWhich xs
findWhich [(B char):xs] = 1 + findWhich xs
