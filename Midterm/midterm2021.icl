...
tuples_with_propertyAux :: [(Int,Int)] -> Int
tuples_with_propertyAux list = length [x \\ x <- list | fst x > snd x] 

tuples_with_property :: [[(Int,Int)]] -> [Int]
tuples_with_property list = [ tuples_with_propertyAux x \\ x <- list ]
...
decreasing_list n = map (\x = [x,x-1..1] ++ [1..x] ) [n,n-1..1] ++ map (\x = [x,x-1..1] ++ [1..x] ) (reverse [n,n-1..1])
...
matchIndex :: [[a]] (Int -> Bool) -> Bool
matchIndex lists function = length [x \\ x <- lists & i <- [1..length lists] |  function (length x) == function i ] == length lists

//func :: [[a]] (Int -> b) -> Bool | == b
//func list f = filter ((==) False) [ f (length sublist) == f index  \\ sublist <- list & index <- [1..] ] == []
...
max_even_sumAux :: [[Int]] -> [Int]
max_even_sumAux list = [ sum x \\ x <- list]

max_even_sum :: [[Int]] -> Int
max_even_sum [] = -1
max_even_sum list = if ([x \\ x <- max_even_sumAux list | isEven x ] == []) (-1) (maxList [x \\ x <- max_even_sumAux list | isEven x ])

//maxeven :: [[Int]] -> Int
//maxeven list =  maxList (filter isEven (map sum list))
...
isPalindromic n = n == ListToNum (reverse (NumToList n))

extractAux :: [Int] -> (Int,Int)
extractAux list = (ListToNum [x \\ x <- list | isPrime x], ListToNum [x \\ x <- list | isPalindromic x])

extract :: [[Int]] -> [(Int, Int)]
extract [] = []
extract [x:xs] = [extractAux x : extract xs]
...
PrimeDiv :: Int -> [Int]
PrimeDiv n = [x \\ x <- [2..n/2] | n rem x == 0 && isPrime x]
...
phi :: Int -> Int
phi n = length [x \\ x <- [1..n] | phiAux x (PrimeDiv n) ]
...
primeMulti :: Int Int Int -> Int
primeMulti a b c
| a rem b == 0 = primeMulti (a/b) b (c+1)
= c


prime_factors_mult :: Int -> [(Int, Int)]
prime_factors_mult n = sort [ (x,primeMulti n x 0) \\ x <- [n,n-1..2] | isPrime x && (n rem x == 0) ]
...
intersection [] (c, d) = []
intersection [(a, b):xs] (c, d)
| ((a >= c) && (a <= d)) || ((b >= c) && (b <= d)) || ((a <= c) && (b >= d)) = [(a, b): intersection xs (c, d) ]
= intersection xs (c, d)
...
tuple_sortAux :: [Int] -> [(Int,Int,Int)]
tuple_sortAux [] = []
tuple_sortAux [a,b,c:xs] = [(a,b,c): tuple_sortAux xs]

tuple_sort :: [(Int,Int,Int)] -> [(Int, Int, Int)]
tuple_sort list = tuple_sortAux (sort (flatten [[a] ++ [b] ++ [c] \\ (a,b,c) <- list]))
