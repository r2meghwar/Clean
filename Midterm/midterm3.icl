...
parasitic n x = ListToNum ([x] ++ (NumToList (n/10))) == n * x

//Start = parasitic 102564 4 // True
//Start = parasitic 142857 5 // False
...
doubleOne :: [Int] -> [Int]
doubleOne list = [x \\ x <- list | Occurrence 1 (NumToList x) >= 2]
...
AminusB :: [Int] [Int] -> [Int]
AminusB listA listB = [x \\ x <- listA | (isMember x listA) && not (isMember x listB)]

difference :: [[Int]] [[Int]] -> [[Int]] 
difference listA listB = [ AminusB x y \\ x <- listA & y <- listB ]
...
MidItem :: [Int] Int -> [Int]
MidItem list n = take a list ++ [n] ++ drop (a+1) list
where a = (length list)/2

repMid :: [[Int]] Int -> [[Int]]
repMid list n = [ MidItem x n \\ x <- list]
...
holdsTrue [] = False
holdsTrue list = length [x \\ x <- list | isEven (fst x) && isOdd (snd x)] == length list
...
superDigit n
| n <= 9 = n
= superDigit (sum (NumToList n)) 

super_digit :: [Int] -> [Int]
super_digit list = [ superDigit x \\ x <- list ]
...
powersList list n = [x \\ x <- list , i <- [0..last list] | x == n^i ]
...
twinPrimes a b = length [(x,y) \\ x <- [a..b] & y <- [a+2..b] | isPrime x && isPrime y]
